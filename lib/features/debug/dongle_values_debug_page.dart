...
import '../../core/services/dongle_trace_log_service.dart';
...
final RegExp _traceLinePattern = RegExp(r'^K-LINE \[(.+?)\] SONUÇ: (.+)$');
...
class DongleValuesDebugPage extends StatefulWidget {
  const DongleValuesDebugPage({super.key});

  @override
  State<DongleValuesDebugPage> createState() => _DongleValuesDebugPageState();
}

class _DongleValuesDebugPageState extends State<DongleValuesDebugPage> {
  bool _paused = false;
  ...
  void _togglePause() {
    setState(() {
      if (_paused) {
        ...
        _frozenLines = List<String>.from(DongleTraceLogService.instance.lines);
        _paused = true;
      }
    });
  }
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      await KLineLogExportService.exportAndShare(
        DongleTraceLogService.instance.lines,
      );
    } catch (e) {
      if (mounted) {
        showAppSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Dongle — Ham Değerler'),
        actions: [
          IconButton(
            icon: _exporting
                ? const SizedBox(
          ...
          IconButton(
            ...
            tooltip: 'Temizle',
            onPressed: () => DongleTraceLogService.instance.clear(),
          ),
        ],
      ),
      body: Column(
        ...
        children: [
          Padding(
            ...
            child: Column(
              ...
              children: [
                const Text(
                  'Canlı İletişim Kaydı (Dongle)',
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
                ),
                const SizedBox(height: 4),
                Text(
                  'Dongle üzerinden "Bağlan" ile bağlan — her alan okunduğu anda burada gerçek, parse edilmiş değeriyle görünür.',
                  style: TextStyle(
                    fontSize: 11,
  ...
  Widget _buildLiveTrace(BuildContext context) {
    ...
    return ListenableBuilder(
      listenable: DongleTraceLogService.instance,
      builder: (context, _) =>
          _fieldTable(context, DongleTraceLogService.instance.lines),
    );
  }
  ...
  Widget _fieldTable(BuildContext context, List<String> lines) {
    ...
    if (rows.isEmpty) {
      return Center(
        child: Text(
          'Henüz kayıt yok — dongle\'a bağlanılmadı veya el sıkışma başlamadı.',
          style: TextStyle(
            fontSize: 12,
            color: Theme.of(context).colorScheme.outline,
          ),
